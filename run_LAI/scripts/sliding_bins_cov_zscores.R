# This script computes sliding-window averages of HGall values per SNP,
# calculates Z-scores per dataset, and produces covariance-weighted Z-scores across datasets.
# Outputs:
#   - combined_weighted_z_scores.txt
#   - combined_avg_zscore_plot.pdf
#   - combined_avg_qq.pdf
...
read_and_process_dataset <- function(file_path, dataset_name) {
  ...
  data_input <- data_input[order(data_input$CHR, data_input$POS), ]
  
  binned_data_list <- list()
  
  for (chr in unique(data_input$CHR)) {
    chr_data <- data_input[data_input$CHR == chr, ]
...
for (i in 1:num_datasets) {
  ...
  z_scores_matrix[[i]] <- dataset_data$new_z
}

# Combine Z-scores into a matrix
z_scores_matrix <- do.call(cbind, z_scores_matrix)
cov_matrix <- cov(z_scores_matrix, use = "pairwise.complete.obs")
...
ggplot(combined_data, aes(x = index, y = weighted_z, color = color)) +
  ...
  geom_hline(yintercept = c(3, 3.5, -3, -3.5), color = "blue") +
