...
#num hg *2
printf "%s " $(yes 2 | head -n 6) >> classes.txt
...
#the number we want 
awk '{print substr($0, 1, length($0)-90)}' /project/mathilab/gmies/neolithic_selection/allentoft_data/031825_rerun_filter_allen_LAI/results/rfmix_sep/rfmix_alleles${chr}.txt > rfmix_alleles${chr}.txt
