...
#ind_df <- read_table("/project/mathilab/gmies/neolithic_selection/qpadm/0100825_allen
toft_qpadm/output_data/qpadm_input.ind", col_names = FALSE)
...
results_list <- map(targets, function(target) {
  ...
  f2_blocks <- f2_from_geno("qpadm_input", pops = pops_for_target)
